...
typedef struct {
    ...
    bool control_sent;      // Whether we've already sent a control key event
} CapsLockState;
 ...
 * Determines whether to send Escape (short press) or release Control (long press/combo)
 */
void handle_caps_lock_release() {
    if (!caps_state.is_pressed) return;
    
    uint64_t release_time = mach_absolute_time();
    uint64_t hold_duration = mach_time_to_milliseconds(release_time - caps_state.press_time);
 ...
 * Processes keyboard input and routes Caps Lock and other key events appropriately
 * 
 ...
 */
void hid_input_callback(void* context __attribute__((unused)), IOReturn result __attribute__((unused)), void* sender __attribute__((unused)), IOHIDValueRef value) {
 ...
 * 
 * @return A CFMutableDictionaryRef for keyboard device matching, or NULL on failure
 */
CFMutableDictionaryRef create_keyboard_matching_dictionary() {
    ...
    if (dict) {
        ...
        CFNumberRef usage_page = CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &(int){kHIDPage_GenericDesktop});
        CFNumberRef usage = CFNumberCreate(kCFAllocatorDefault, kCFNumberIntType, &(int){kHIDUsage_GD_Keyboard});
 ...
 * Configures device matching, callbacks, and opens the HID manager for input monitoring
 */
