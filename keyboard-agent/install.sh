...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "Usage: $0"
    echo
    echo "Installs keyboard_agent as a LaunchAgent (user-level service)"
    echo "Installs to ~/.local/bin and runs as user for Input Monitoring"
    echo
    exit 0
fi
...
sed "s|INSTALL_DIR_PLACEHOLDER|$INSTALL_DIR|g" "$PLIST_NAME" > "$PLIST_DIR/$PLIST_NAME"

echo "Installation complete."
echo
echo "NOTE: The agent is installed to ~/.local/bin"
echo "Make sure ~/.local/bin is in your PATH by adding this to your shell profile:"
...
if launchctl list | grep -q "keyboard_agent"; then
    ...
    echo "Check logs with: tail -f /tmp/keyboard_agent.log"
fi
...
echo ""
echo "Right-Option Arrow Keys:"
echo "• Right-Option + H: Left arrow"
echo "• Right-Option + J: Down arrow"
echo "• Right-Option + K: Up arrow"
echo "• Right-Option + L: Right arrow"
echo
