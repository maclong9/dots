...
setup_colors() {
  ...
  for scheme_dir in "$HOME/.config/colors"/*; do
    ...
    [ "$IS_MAC" = true ] && {
      process_colorscheme_files "$scheme_dir" "*.xccolortheme" "$HOME/Library/Developer/Xcode/UserData/FontAndColorThemes" "Xcode" || {
...
setup_touch_id() {
  ...
  [ -f /etc/pam.d/sudo_local ] && grep -q "^auth.*pam_tid.so" /etc/pam.d/sudo_local && {
...
setup_ssh() {
  ...
  # Start ssh-agent
  eval "$(ssh-agent -s)" >/dev/null 2>&1 || log warning "Failed to start ssh-agent"
...
build_container() {
  ...
  local pkg_url="https://github.com/apple/container/releases/download/0.1.0/container-0.1.0-installer-signed.pkg"
  local pkg_file="container-installer.pkg"

  log debug "Downloading package from $pkg_url"
  
  spinner "Downloading container package" curl -L -o "$pkg_file" "$pkg_url" || {
    log error "Failed to download container package"
    return 1
  }

  spinner "Installing container package" sudo installer -pkg "$pkg_file" -target / || {
    log error "Failed to install container package"
    rm -f "$pkg_file"
    return 1
  }

  rm -f "$pkg_file" || log warning "Failed to remove downloaded package file"
